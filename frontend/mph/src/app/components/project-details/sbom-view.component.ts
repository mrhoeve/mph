  ...
  template: `
    <div class="sbom-item">
      <div class="item-content" [class.transitive]="level > 0">
        <span
          class="expand-icon"
          *ngIf="component.dependencies.length > 0"
        ...
        <span class="artifact-id"
          ><strong>{{ component.artifactId }}</strong></span
        >
      ...
      @if (expanded() && component.dependencies.length > 0) {
        <div class="children">
          @for (
            dep of component.dependencies;
            track dep.groupId + ':' + dep.artifactId + ':' + dep.version
          ) {
  ...
  template: `
    <div class="sbom-view">
      ...
      @if (loading()) {
          ...
          @if (viewMode() === 'tree') {
            @if (searchTerm()) {
              <table class="sbom-table">
                ...
                <tbody>
                  @for (
                    comp of filteredComponents();
                    track comp.groupId + ':' + comp.artifactId + ':' + comp.version
                  ) {
                    <tr>
                      <td>{{ comp.groupId }}</td>
                      <td>{{ comp.artifactId }}</td>
                      <td>
                        <span class="version-tag">{{ comp.version }}</span>
                      </td>
                      <td>
                        <span class="scope-tag" *ngIf="comp.scope">{{ comp.scope }}</span>
                      </td>
                    </tr>
                  } @empty {
                    <tr>
                      <td colspan="4" class="empty-results">
        ...
        <p class="component-count" *ngIf="viewMode() === 'tree'">
          {{ totalComponents() }} components found
        </p>
      }
    </div>
  `,
...
export class SbomViewComponent implements OnInit, OnChanges {
  ...
  protected readonly filteredComponents = computed(() => {
    ...
    return this.flatComponents().filter(
      (c) =>
